...
def func_to_get_desired_features(filename):
    ...
    # # Checks to see if the artist name and song title are part of the processed labeled data 
    ...
    #     # Grab the right song, because sometimes there are duplicate song titles. This makes sure that we grab the right data (both the right song title and artist).
    #     for i in index_title:
    #         if i in index_artist:
    #             index = i
    #             break
        
    #     # This is to ensure that we don't add any new data points. We just want to add new features.
    #     try:
    #         desired_columns['Genre'].append(labeled_data['Genre'].values[index])
    #     except UnboundLocalError:
    #         print(artist_name.decode('UTF-8'), "with song", title.decode('UTF-8'), "not in Labeled_Data_Final.csv")
    #         return
