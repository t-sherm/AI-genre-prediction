...
# 2) New random forest classifier (with optimized hyperparameters -- using results from GridSearchCV)
rnd_clf_optimized = RandomForestClassifier(n_estimators=200, min_samples_leaf=100, min_samples_split=100, max_features=50, max_depth=20, criterion='gini', random_state=42)
rnd_clf_optimized.fit(X_train, y_train)

# Accuracy score (With optimized hyperparameters)
print("Random Forest Classifier (Optimized Hyperparameters) -- Train Set:", rnd_clf_optimized.score(X_train, y_train)) # 0.35032965914008607
print("Random Forest Classifier (Optimized Hyperparameters) -- Test Set:", rnd_clf_optimized.score(X_test, y_test)) # 0.30872619896140924
...
print("Random Forest Classifier (Optimized Hyperparameters and Bagging) -- Train Set:", rnd_clf_bagging.score(X_train, y_train)) # 0.331110149428506
print("Random Forest Classifier (Optimized Hyperparameters and Bagging) -- Test Set:", rnd_clf_bagging.score(X_test, y_test)) # 0.3033295998370838
...
print("Extra Trees Classifier (Optimized Hyperparameters) -- Train Set:", rnd_clf_extra_trees.score(X_train, y_train)) # 0.3254079372756663
print("Extra Trees Classifier (Optimized Hyperparameters) -- Test Set:", rnd_clf_extra_trees.score(X_test, y_test)) # 0.2901944812137257 
